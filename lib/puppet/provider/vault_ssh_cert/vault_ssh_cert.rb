...
Puppet::Type.type(:vault_ssh_cert).provide(:vault_ssh_cert) do
  ...
  def self.instances
    ...
    cert_filenames.each do |cert_file|
      ...
      begin
        expiration = self.get_expiration cert_file
      rescue => error
        expiration = nil
      end
  ...
  def self.prefetch(resources)
    instances.each do |prov|
      puts "checking #{prov.name}: #{resources[prov.name]}"
  ...
  def self.get_expiration(file)
    expiration = nil
    output = ssh_keygen("-L", "-f", file).chomp
    output.each_line do |line|
      matches = /\s+Valid:\s+from .* to (.*)/.match(line)
      next unless matches
  ...
  def issue_cert
    ...
    request_data = {
      ...
      :ttl              => @resource[:ttl],
      :valid_principals => @resource[:valid_principals],
    }.select {|k, v| !v.nil? }
